...
// #include "Engine/Private/KismetTraceUtils.h" @LGPU 5.4 error wrong path
...
FAutoConsoleVariableRef CVar_DrawCameraDebugInfo(TEXT("SP.DrawCameraDebugInfo"), DrawCameraDebugInfo, TEXT("True to draw camera debugging info."), ECVF_Cheat);
...
FQuat USPCam_ThirdPerson::GetAutoFollowPivotToWorldRotation(const AActor* FollowActor) const
{
	FRotator const ActorRot = FollowActor ? FollowActor->GetActorRotation() : FRotator::ZeroRotator;
	return ActorRot.Quaternion();
}

float USPCam_ThirdPerson::GetViewTargetMeshHeightOffset(const AActor* ViewTarget) const
{
...
FTransform USPCam_ThirdPerson::GetViewTargetToWorld(const AActor* ViewTarget) const
{
	return ViewTarget ? ViewTarget->GetActorTransform() : FTransform::Identity;
}

FTransform USPCam_ThirdPerson::ComputeCameraToWorld(const AActor* ViewTarget, FTransform const& PivotToWorld) const
{
...
void USPCam_ThirdPerson::ComputePredictiveLookAtPoint(FVector& LookAtPointOutput, const AActor* ViewTarget, float DeltaTime)
{
	LookAtPointOutput = LookAtPointOutput + ViewTarget->GetVelocity() * PredictiveLookatTime;
}

FVector USPCam_ThirdPerson::ComputeWorldLookAtPosition(const FVector IdealWorldLookAt, float DeltaTime)
{
	return LookatWorldSpaceInterpolator.Eval(IdealWorldLookAt, DeltaTime);
}

FRotator USPCam_ThirdPerson::ComputeSmoothPivotRotation(const FRotator IdealPivotToWorldRot, float DeltaTime)
{
...
FTransform USPCam_ThirdPerson::ComputePivotToWorld(const AActor* ViewTarget) const
{
	const AController* const PC = PlayerCamera ? Cast<APlayerController>(PlayerCamera->PCOwner) : nullptr;
	if (PC == nullptr)
	{
	...
	else if (AutoFollowMode == ECameraAutoFollowMode::LazyFollow)
	{
		if (!bSkipNextInterpolation)
		{
			...
			else
			{
				...
				{
					// we are essentially inserting another transform here, like a LazyFollowPivot which is a parent to the actual pivot, but with only a pitch control
					// we have to subtract it out, do the lazy follow calculation, then insert it back in. if we don't subtract it, there will be an accumulation feedback loop
					FRotator LastUnsmoothedPivotRot = LastUnsmoothedPivotToWorld.Rotator();
					LastUnsmoothedPivotRot.Pitch -= LazyFollowGoalPitch;		
					
					const FVector LastPivotDir = -LastUnsmoothedPivotRot.Vector();
					...
					NewPivotRot.Pitch += LazyFollowGoalPitch;
				}
		...
		{
			// skipping interpolation, snap to the goal pitch
			FRotator PivotRot = PivotToWorld.Rotator();
			PivotRot.Pitch = LazyFollowGoalPitch;
			PivotToWorld.SetRotation(PivotRot.Quaternion());
		}
	}
...
void USPCam_ThirdPerson::UpdateCamera(class AActor* ViewTarget, UCineCameraComponent* CineCamComp, float DeltaTime, FTViewTarget& OutVT)
{
	...
	if (World)
	{
		...
		FTransform SmoothedPivotToWorld = PivotToWorld;
		{
			...
			if (bSkipNextInterpolation)
			{
				...
				ExtraPivotLocZInterpolator.Reset();
			}

			FVector SmoothedPivotLoc = PivotLocInterpolator.Eval(PivotToWorld.GetLocation(), DeltaTime);
			SmoothedPivotLoc.Z = ExtraPivotLocZInterpolator.Eval(SmoothedPivotLoc.Z, DeltaTime);
...
void USPCam_ThirdPerson::OnBecomeActive(AActor* ViewTarget, USPCameraMode* PreviouslyActiveMode, bool bAlreadyInStack)
{
...
void USPCam_ThirdPerson::SkipNextInterpolation()
{
...
FTransform USPCam_ThirdPerson::GetCameraToPivot(const AActor* ViewTarget) const
{
...
void USPCam_ThirdPerson::PreventCameraPenetration(AActor* Target, TArray<FPenetrationAvoidanceRay>& Rays, const FVector& SafeLoc, const FVector& IdealCameraLoc, float DeltaTime, FVector& OutCameraLoc, float& DistBlockedPct, bool bSingleRayOnly)
{
